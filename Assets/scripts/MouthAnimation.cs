...
public class MouthAnimation : MonoBehaviour
{
    ...
    // Maximum y-position for lipsLow bone
    public float maxLowPosition = 0.2f; // Adjusted to be positive

    // Maximum y-position for lipsTop bone
    public float maxUpPosition = 0.5f;

    // Audio source for playing the audio clip
    private AudioSource audioSource;
    ...
    void Update()
    {
        ...
        // Calculate new positions for the mouth bones based on amplitude
        float lipsTopY = Mathf.Lerp(0.0f, maxUpPosition, amplitude);
        float lipsLowY = Mathf.Lerp(0.0f, maxLowPosition, amplitude); // Adjusted to use maxLowPosition directly
